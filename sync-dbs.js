...
const baseUrl = 'postgresql://postgres:dYQZOQCUTSQxeZFliQfHfDMzkMGEZFQM@reseau.proxy.rlwy.net:48194';
const databases = ['postgres', 'railway'];
...
async function syncDatabase(dbName) {
  ...
  try {
    ...
    const existing = await pool.query('SELECT COUNT(*) as c FROM menu_items');
    if (parseInt(existing.rows[0].c) === 0) {
      ...
      console.log(`Already has ${existing.rows[0].c} menu items, skipping seed`);
    }

    // Show table counts
    const counts = await pool.query(`
      SELECT 'reservations' as t, COUNT(*) as c FROM reservations UNION ALL
  ...
  } catch (e) { console.error(`ERROR ${dbName}:`, e.message); }
  finally { pool.end(); }
}
